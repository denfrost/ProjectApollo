...
#include "ActorCoverPointGeneratorTask.h"
...
const bool FActorCoverPointGeneratorTask::FindGroundPoint(FVector& OutGroundPoint, const FVector Location) const
{
	...
	CollisionQueryParams.bFindInitialOverlaps;
	CollisionQueryParams.AddIgnoredActor(Owner);
	CollisionQueryParams.TraceTag = "CoverGenerator_FindGroundPoint";

	bool bResult = World->LineTraceSingleByChannel(OutHit, Location, Location - FVector(0.0f, 0.0f, ScanGridUnit),
	                                               ECollisionChannel::ECC_GameTraceChannel5, CollisionQueryParams);

	OutGroundPoint = OutHit.ImpactPoint;
	return bResult && !OutHit.bStartPenetrating;
}

void FActorCoverPointGeneratorTask::GatherFreeGridPoints(TArray<FVector>& OutGridPoints, const FVector BlockedGridPoint,
                                                         const TArray<FVector>& FreeGridPoints,
                                                         const float NavPointEqualityTolerance)
{
	for (const FVector& GridPoint : FreeGridPoints)
	{
		if (FMath::IsNearlyEqual(GridPoint.X, BlockedGridPoint.X, NavPointEqualityTolerance) &&
			FMath::IsNearlyEqual(GridPoint.Y, BlockedGridPoint.Y, NavPointEqualityTolerance) && FMath::IsNearlyEqual(
				GridPoint.Z, BlockedGridPoint.Z, NavPointEqualityTolerance))
		{
			bool bUnique = true;
			for (const FVector& CoverPoint : OutGridPoints)
			{
				if (FMath::IsNearlyEqual(GridPoint.X, CoverPoint.X, NavPointEqualityTolerance) &&
					FMath::IsNearlyEqual(GridPoint.Y, CoverPoint.Y, NavPointEqualityTolerance) && FMath::IsNearlyEqual(
						GridPoint.Z, CoverPoint.Z, NavPointEqualityTolerance))
				{
					bUnique = false;
					break;
				}
...
void FActorCoverPointGeneratorTask::DoWork()
{
	...
	if (UApolloCoverSystemLibrary::bShutdown)
		return;
	bDebugDraw = UApolloCoverSystemLibrary::GetInstance(World)->bDebugDraw;
#endif
	...
	if (bGeneratePerStaticMesh) // collect the bounding boxes of all the static meshes of Owner
		...
		if (UApolloCoverSystemLibrary::bShutdown)
			return;
		UApolloCoverSystemLibrary::GetInstance(World)->AddCoverPoints(coverPoints);

		DEC_DWORD_STAT(STAT_TaskCount);
}

FActorCoverPointGeneratorTask::FActorCoverPointGeneratorTask(AActor* _Owner, UWorld* _World,
                                                             float _BoundingBoxExpansion, float _ScanGridUnit,
                                                             float _SmallestAgentHeight, bool _bGeneratePerStaticMesh)
{}